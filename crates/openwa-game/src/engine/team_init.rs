...
//!
//! Pure Rust ports of three sub-functions called once during DDGame setup:
//! - `CGameTask__InitTeamScoring` (0x528510)
//! - `CGameTask__InitAllianceData` (0x5262D0)
//! - the team-name CPU-marker check baked into `DDGame__InitGameState`
...
pub unsafe fn init_team_scoring(wrapper: *mut DDGameWrapper) {
    unsafe {
        let game_info = (*(*wrapper).ddgame).game_info;
...
/// 6-entry per-team scoring flags on DDGame, and an auxiliary alliance ID
                    ...
                    *scoring_flag = override_val as u32;
                    ...
                    if num_teams_byte == 0 || override_byte != 0 {
                        *scoring_flag = 1;
                        ...
                        let starting_bitmask =
                            (*wrapper)._alliance_bitmasks[starting_team as usize];
                    ...
                    if alliance_group != starting_team2 && seen[alliance_id as usize] == 0 {
                        let starting_bitmask2 =
                            (*wrapper)._alliance_bitmasks[starting_team2 as usize];
...
///
/// Inlined within the original `DDGame__InitGameState` (0x526500). Sets
/// `DDGame.team_color` from the GameInfo source byte, then overrides to -1
/// if any team name's first byte has bit 7 set (CPU marker).
pub unsafe fn init_team_color_from_names(ddgame: *mut DDGame, game_info: *mut GameInfo) {
    unsafe {
