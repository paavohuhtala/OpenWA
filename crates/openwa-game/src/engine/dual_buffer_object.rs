//! DualBufferObject — dual-buffer container used by GameRuntime for state
//! serialization (`main_buffer`, `state_buffer`).
//!
//! Distinct from [`BufferObject`](super::buffer_object::BufferObject):
//! that's the per-ring queue type with `classify_input_msg` / `alloc_slot`
//! methods used by `render_buffer_a/b` and other GameRuntime ring slots.
//! This is the outer 0x48-byte container with two parallel sub-buffer
//! headers — name matches Ghidra's `DualBufferObject__Constructor`.
//!
...
/// Allocate a DualBufferObject (0x48 bytes) with both sub-buffer sizes
/// computed from `GameInfo`.
///
/// Pure Rust port of `DualBufferObject__Constructor` (0x00545FD0). The
/// original is a usercall with the secondary buffer's capacity passed
/// implicitly in EDI.
pub unsafe fn allocate_dual_buffer_object(
    world: *mut GameWorld,
