...
fn raster_hmajor(
    ...
    let px_start = start.to_int();

    for px in px_start..=px_start + count {
...
fn raster_vmajor(
    ...
    let count = (end - start).to_int();
    let py_start = start.to_int();

    for py in py_start..=py_start + count {
...
fn raster_thick_hmajor_wide(
    ...
    for px in start.to_int()..start.to_int() + count {
        let py = y.to_int();
...
fn raster_thick_vmajor_wide(
    ...
    for py in start.to_int()..start.to_int() + count {
        let px = x.to_int();
        writer.put_pixel_clipped(px, py - 1, color);
        writer.put_pixel_clipped(px + 1, py - 1, color);
        writer.put_pixel_clipped(px - 1, py, color);
