...
- **`openwa-core`** — Cross-platform, idiomatic Rust fundamentals. No WA.exe memory references, no Ghidra addresses, no Windows APIs. Modules migrate here from `openwa-game` as they're confirmed portable. Currently empty — see `crates/openwa-core/CLAUDE.md` for the charter.
