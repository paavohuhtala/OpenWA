...
- **`Task` trait**: Provides `task()`, `world()`, `as_task_ptr()`, `as_task_ptr_mut()`, and `broadcast_message()` on all BaseEntity subclasses. Eliminates `.base.base` chains. Implemented for all task types in `task/mod.rs`.
