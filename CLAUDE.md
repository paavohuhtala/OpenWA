...
- **`openwa-dll`** — Injected DLL (`openwa.dll`): thin hook installation shims that wire core's game logic into WA.exe via MinHook. Most hooks come from declarative `hooks/*.toml` joined against `re/**/*.toml` by `openwa-re-codegen` — see `crates/openwa-dll/CLAUDE.md` for the codegen path and the hand-written fallbacks (vtable replacements, traps, bridges). Logs to `OpenWA.log`. Runs registry-driven startup checks automatically at load.
...
**Env vars:** `OPENWA_WATCH_FRAME=N` (arm at frame N), `OPENWA_WATCH_WRAPPER=1` (base=GameRuntime), `OPENWA_WATCH_DISPLAY=1` (base=display object).
...
- **Constructors are usually `__stdcall`** — `this` passed on stack, not ECX. **Exception**: MissileEntity constructor (0x507D10) is `__thiscall` (ECX=this, 3 stack params, RET 0xC). Always verify by checking the call site's register setup.
...
- **Generic BaseEntity<V> for typed vtables**: `BaseEntity<V: Vtable = *const c_void>` and `WorldEntity<V: Vtable = *const c_void>` take a vtable pointer type parameter. Subclasses specify their typed vtable: `TeamEntity { base: BaseEntity<*const TeamEntityVtable> }`. The `Vtable` marker trait is auto-implemented by the `#[vtable]` macro. `FieldRegistry` derive supports generics by substituting type params with their defaults for `offset_of`/`size_of`.
- **`Entity` trait**: Provides `entity()`, `world()`, `as_entity_ptr()`, `as_entity_ptr_mut()`, and `broadcast_message()` on all BaseEntity subclasses. Eliminates `.base.base` chains. Implemented for all entity types in `entity/mod.rs`.
- **`broadcast_message()` / `broadcast_message_raw()`**: Pure Rust port of BaseEntity::HandleMessage (0x562F30). Iterates the sparse children array and calls each child's vtable[2]. Uses `read_volatile` for `children_watermark` and `children_data` — **required** because LLVM caches these reads across virtual dispatch calls even through `*mut`. Prefer `BaseEntity::broadcast_message_raw(ptr, ...)` over `(*ptr).broadcast_message(...)` — see noalias rule above.
- **BaseEntity base vtable**: 7 slots (not 8). ProcessFrame is slot 6. Slots 7+ are class-specific extensions. WorldEntity adds no virtuals.
- **Noalias rule — ALWAYS use `_raw` methods on WA objects**: `bind_!` methods with `&mut self` give LLVM noalias guarantees that WA bridge calls violate, causing silent miscompilation. **Always use `Type::method_raw(ptr, args)` instead of `(*ptr).method(args)`**. Similarly use `BaseEntity::world_raw(ptr)` instead of `(*ptr).world()`, and `BaseEntity::broadcast_message_raw(ptr, ...)` instead of `(*ptr).broadcast_message(...)`. For `as_entity_ptr`, just cast: `worm as *mut BaseEntity`.
